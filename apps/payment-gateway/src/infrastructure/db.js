...
export const db = {
  ...
  /**
   * Метод для управления транзакциями.
   * Возвращает клиент, который будет "закреплен" за текущим процессом.
   */
  async getTransactionClient() {
    const client = await pool.connect();

    return {
      // Проксируем запрос к конкретному клиенту
      query: (text, params) => client.query(text, params),
      // Метод для возврата клиента в пул
