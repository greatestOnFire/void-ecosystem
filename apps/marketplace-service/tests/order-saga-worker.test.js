...
test('OrderSagaWorker: должен перехватить команду CANCEL_TRANSFER и перевести заказ в статус CANCELED', async () => {
	const mockRedisSub = new EventEmitter();
	mockRedisSub.subscribe = async () => {};
	...
	// Имитируем команду отмены Саги от платежного шлюза при сбое платежа
	mockRedisSub.emit('message', 'payment-events', JSON.stringify({
		type: 'CANCEL_TRANSFER',
		payload: {
			originalReferenceId: 'order-fail-999'
		}
	}));
	
	await new Promise(setImmediate);
	
	// Проверяем строгий финтех-инвариант Саги: статус обязан стать CANCELED
	assert.strictEqual(updatedOrderId, 'order-fail-999');
	assert.strictEqual(updatedStatus, 'CANCELED');
});
